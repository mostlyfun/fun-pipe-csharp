...
public readonly struct Opt<T>
{
    ...
    /// </summary>
    public bool Equals(Opt<T> other) => this == other;
    /// <summary>
    /// Returns whether <paramref name="first"/> is equal to <paramref name="second"/>.
    /// </summary>
    public static bool operator ==(Opt<T> first, Opt<T> second) => first.IsNone ? second.IsNone : second.IsSome && first.value.Equals(second.value);
    /// <summary>
    /// Returns whether <paramref name="first"/> is not equal to <paramref name="second"/>.
    /// </summary>
    public static bool operator !=(Opt<T> first, Opt<T> second) => first.IsNone ? second.IsSome : second.IsNone || !first.value.Equals(second.value);
    /// <summary>
    /// Returns whether this option is equal to the <paramref name="obj"/>.
    /// </summary>
    public override bool Equals(object obj) => (obj is Opt<T>) ? (this == (Opt<T>)obj) : false;
    /// <summary>
    /// Serves as the default hash function.
    /// </summary>
    public override int GetHashCode() => IsNone ? int.MinValue : value.GetHashCode();
    /// <summary>
    ...
    /// </summary>
    public bool Equals(Res<T> other) => IsSome && other.IsOk && value.Equals(other.Unwrap());
}
